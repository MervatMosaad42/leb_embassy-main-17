...
class DocumentsCustom(models.Model):
    ...
    document_owner_name = fields.Char(
        string='If Others Document Owner Name',
        required=False
    )
    document_owner_description = fields.Char(
        string='Document Owner Description',
        required=False
    )
    ...
    def _check_document_number(self):
        ...
        for rec in self:
            doc = self.env['documents.document'].search(
                    [('document_number', '=', rec.document_number), ('active', '=', True), ('id', '!=', rec.id)])
            # if len(doc) > 0:
    ...
    def create(self, vals):
        ...
        vals['folder_id']= self.env['documents.folder'].search([('is_default_folder', '=', True)], limit=1).id
